...
def test_e2e_update_flow(mock_clients):
    ...
    with (
        patch("builtins.input", side_effect=["Fix the text", "exit"]),
...
def test_e2e_delete_flow(mock_clients):
    ...
    mock_gemini.analyze_and_act.return_value = {"action": "DELETE", "target_block_index": 0}

    mock_notion.delete_block.return_value = True

    with (
        patch("builtins.input", side_effect=["Delete it", "exit"]),
        patch("sys.argv", ["notion_sidecar"]),
        patch.dict(
            "os.environ", {"NOTION_TOKEN": "fake", "PAGE_ID": "fake", "GEMINI_API_KEY": "fake"}
        ),
...
def test_e2e_insert_flow(mock_clients):
    ...
    with (
        patch("builtins.input", side_effect=["Insert after this", "exit"]),
        patch("sys.argv", ["notion_sidecar"]),
        patch.dict(
            "os.environ", {"NOTION_TOKEN": "fake", "PAGE_ID": "fake", "GEMINI_API_KEY": "fake"}
        ),
